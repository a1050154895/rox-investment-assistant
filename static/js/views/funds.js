...
ROX.register('/funds', async function(container, params) {
  ...
  container.innerHTML = `
    <div class="fund-page">
      ...
      <div class="card fund-chart-card"><div class="card-header"><div><div class="card-title">价格观察</div><div class="card-subtitle">用于观察交易价格波动，不替代净值分析</div></div><span class="tag ${fund.stale ? 'tag-amber' : 'tag-green'}">${fund.stale ? '快照' : '实时'}</span></div><div id="fund-kline" class="fund-kline"></div></div>
