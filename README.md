...
> 桌面端的信息密度与研究卡片布局已基本稳定；手机端目前仅完成基础响应式，
> 触控尺寸、文字密度、滑块、图表交互和底部导航仍需系统适配，详见
> [移动端现状与适配计划](#移动端现状与适配计划)。

## 移动端现状与适配计划

当前状态：桌面端体验较好，手机端有明显不便，属于下一阶段的重点改造对象。
...
| 横向布局 | 表格、指标网格仍会横向溢出 | 表格改卡片/抽屉，指标改两列自适应 |
| 弹窗与底部导航 | 弹窗高度、底部导航遮挡内容 | 使用安全区与 `100dvh`，弹窗支持滚动吸底操作 |
...
| 移动端 | 仅基础响应式；触控尺寸、文字密度、滑块、图表交互与底部导航待系统适配 |
| 测试 | 已有 78 个后端测试（可信数据 + API 冒烟 + 研究卡）；尚缺前端自动化测试与 E2E |
