...
## 当前版本状态（v4.43.0 · 2026-08-28）

> 221 项后端测试、10 项移动端 E2E 全绿，Render 线上持续部署。以下为 v4.9.0 以来的主要升级：
...
现有 182 个后端测试覆盖（含回测费用模型数值测试、AI 三层模式、数据契约、知识库、集中度计算），并由 Playwright E2E 与 Lighthouse 移动审计补充前端质量检查：
...
| 测试 | 188 个后端测试、4 个 Playwright 移动端 E2E，并接入 Lighthouse 移动审计 |
