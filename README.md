...
- **快速速记**：借鉴 Gangtise 录音速记，盘中灵感、事件快记、标签筛选、置顶排序，可关联研究卡；研究报告导出纳入研究卡与速记。
...
- **DeepEar 思路吸收**：借鉴其“资讯/价格/判断持续演化”的研究链，盘中异动结果增加新闻匹配状态、最大量价时间点和量价方向代理；不引入其重型多智能体、Kronos 预测或未经验证的预测分数。
...
- **量化能力（重启用+吸收前身 ROX3.0-Quant 资产）**：回测/选股/预警在腾讯实时数据层就绪后诚实重启用；回测引擎 v2（佣金/印花税/滑点/最低佣金、盘中止损止盈、夏普/盈亏因子/平均持仓）；海龟突破与动量策略；通达信风格指标（ZIG/PEAK 等，强制未来函数警示，禁作实时信号）；ETF 价格口径跟踪误差代理。
- **本地知识库**：txt/md/docx 纯标准库索引（docx=zip+xml），关键词+二元组检索，文件不进 git、不外发。
- **移动端左右布局**：左侧抽屉导航+右侧内容（替代 11 项违规底部 Tab），44/48px 触控目标、ESC/遮罩关闭、安全区与 reduced-motion 适配；宽表自动卡片化。
...
- **研究工作台**：吸收 Gangtise 的任务入口与一页通组织方式，统一进入标的研究、主题研究、反证自查、复盘、知识检索和 AI 研究辅助；观察台与异动雷达入口联动；不改变无 AI 模式和可信数据边界。
