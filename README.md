...
## 当前版本状态（v4.32.0 · 2026-08-28）

> 205 项后端测试、6 项移动端 E2E 全绿，Render 线上持续部署。以下为 v4.9.0 以来的主要升级：

- **研究闭环**：跨页面证据抽屉（情报/个股/ETF/宏观一键把证据送入研究卡为事实/反证/待验证）；研究卡 8 态完整生命周期（草稿→研究中→待验证→待决策→观察中→已复盘/已失效/已归档）；研究档案页（关联决策、胜率、复核到期提醒）；产业链三问与反模式自查两个研究模板。
...
- **异动雷达**：ATR 基线 + 波动率突破 + 成交量异动 + 新闻反查；盘前隔夜新闻 × 自选股交叉筛选；异动可直接创建研究卡，把价格信号变成研究线索。不伪造资金流，以成交量/振幅异动作为代理指标。
