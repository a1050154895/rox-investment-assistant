...
## 当前版本状态（v4.50.0 · 2026-09-06）

> 287 项后端测试、15 项移动端 E2E 全绿、15 项移动端 E2E 全绿、15 项移动端 E2E 全绿、15 项移动端 E2E 全绿、15 项移动端 E2E 全绿，Render 线上持续部署。以下为 v4.9.0 以来的主要升级：

- **v4.50 宏观行业漏斗**：新增三段式过滤页 `/funnel`——宏观状态（矩阵分值）→ 行业排序（显式方法论规则匹配 + 东财行业资金流实测）→ 行业内候选样本（东财成分股按流通市值降序）。每一步展示命中规则、证据值与理由，全链路标注「候选池非投资建议」；中性区间诚实返回空排序。宏观指标解析修复：不再假设接口行序，按观察期取最新（修复 M2/PMI 停留 2026-01 的取行错误），并新增 `/api/macro/source-diagnosis` 数据源诊断端点。方法论蒸馏：指数估值分位与盈利-估值归因、行业横向对比矩阵（docs/methodology.md）。
- **v4.49 管理员反馈视图**：新增管理员专属页面（时间 / 用户 / 联系方式 / 来源页面），管理员身份由 `ADMIN_USERNAMES` 环境变量配置，服务端强制校验；`/api/auth/me` 返回管理员标记，设置页对管理员显示运营入口。
- **v4.48 免费公测准备**：新增法律与合规页（用户协议 / 隐私政策 / 风险揭示，/legal 公开可读）；新增账号注销（密码二次确认后不可逆删除全部数据，践行数据删除权）；设置 → 账户新增「导出完整备份 / 意见反馈 / 注销账号」；新增数据库自动备份工作流（GitHub Actions 定时 pg_dump，需配置 DATABASE_URL secret）；新增产品内意见反馈（存库 + 可选邮件转发）。
- **v4.47 进站公告与软件介绍**：登录后按版本自动弹窗提示更新（每版本一次，新用户看新手引导不受打扰）；弹窗含「更新公告 / 软件介绍」双标签；侧栏与顶栏版本号可点击随时重看；公告数据层与版本号由测试强制同步。
- **v4.46 知识库语料与蒸馏扩充**：索引器支持子目录递归与超长文档诚实截断（带目录签名守卫，内容不变不重建）；新增林森池八问（企业长期质量检查）与社区量化策略过拟合自查两条内置指引；思想来源登记见 docs/strategy_origins.md，研究补充原件收于 docs/research-supplements/。
- **v4.45 认知工具与知识库增强**：个股透视新增「多周期 MACD 状态」卡（日/周/月/季各周期独立描述金叉死叉与最近交叉，季线由月线按日历季度聚合，年线样本不足如实标注不可用，只描述历史不构成信号）；研究卡新增「多周期趋势状态自查」「最坏情况先行」两个模板；知识库支持 PDF 索引（pypdf 本地解析）、检索结果带关键词高亮，并内置 10 条人工策展的「操作指引」（与用户文档明确区分来源，可在工作台一键存为速记）。方法论蒸馏与思想来源登记见 docs/methodology.md 与 docs/strategy_origins.md。

- **v4.44 账号找回与邮箱验证**：忘记密码（邮件重置链接 30 分钟有效、单次使用、统一响应防用户名枚举）；「设置 → 账户」绑定/更换邮箱并邮件验证，验证通过后才能用于找回；登录态修改密码；密码变更/重置后旧登录态自动失效。未配置 SMTP 时如实提示"未配置"，开发/测试环境降级为日志投递，不伪造发送成功。

- **研究闭环**：跨页面证据抽屉（情报/个股/ETF/宏观一键把证据送入研究卡为事实/反证/待验证）；研究卡 8 态完整生命周期（草稿→研究中→待验证→待决策→观察中→已复盘/已失效/已归档）；研究档案页（关联决策、胜率、复核到期提醒）；产业链三问、反模式自查、三流纪律三类研究模板。
- **研究对象观察台**：自选对象、实时价格、K线、关联研究卡和决策流同屏联动；无自选与数据不可用时保留诚实空态，不用模拟数据填充。
- **快速速记**：盘中灵感、事件快记、标签筛选、置顶排序，可关联研究卡；研究报告导出纳入研究卡与速记。
- **异动雷达**：ATR 基线 + 波动率突破 + 成交量异动 + 新闻反查；盘前隔夜新闻 × 自选股交叉筛选；5 分钟K线盘中振幅/量能监控；显示最大成交量/振幅时间、量价方向和速度代理；异动可直接创建研究卡，把价格信号变成研究线索。不伪造资金流，以成交量/振幅异动作为代理指标。
- **事件演化线索**：将资讯、价格异动和研究判断放在同一条可追踪链路中，显示新闻匹配状态、最大量价时间点和量价方向代理；不输出未经验证的预测分数。
- **v4.40 事件闭环**：异动事件可记录、刷新和持久化，按观察次数与日期显示首次发现、短期延续、日内持续或次日复核；事件纳入 JSON 备份和 Markdown 报告。
- **宏观口径校验**：实质利率代理严格展示 LPR、CPI、M2、GDP 的季度桶，跨期或缺数据时明确“暂不计算”；不改变原有宏观矩阵评分。
- **UI/UX 与会员边界**：事件跟踪和实质利率进入主页面可见区域；基础研究能力保持开放，付费套餐在支付、订单和权益激活接入前仅显示为规划中。
- **v4.41 移动端交互**：手机端固定“今日/研究/观察/更多”主导航；个股透视按“行情/研究/资金/分析”分区，减少长页面滚动；完整功能保留在“更多”抽屉，底部安全区不遮挡内容。
- **v4.42 页面可用性修复**：补齐速记/异动雷达路由注册，修复决策日志统计路径冲突；基金透视支持代码/名称搜索，认知框架明确“阅读约 X 分钟”，回测无数据时显示可操作的空态。
- **v4.43 认证与移动端入口**：手机端始终显示“登录/账户”入口；未登录时工作台提示可直接打开登录窗口；完善登录中、退出清理和认证流程回归测试。
- **数据可信底座**：统一数据状态契约（realtime/snapshot/stale/unavailable/partial 五态，历史写法出口自动映射）；DataSourceRegistry 数据源健康注册表（真实请求埋点，不做假拨测）+ 今日页健康面板；K线降级链 腾讯→AKShare→新浪直连→历史快照；AKShare 全局限流门。
- **情报升级**：规则式主题主线（时间线/影响行业/验证问题）、突发标记（36 小时+事件词，规则可解释）、登录用户按研究卡/自选关联度排序的个性化情报流；成交额集中度风险温度计（当日实时计算+自建逐日历史，不伪造历史）。
- **AI 三层模式**：不使用 AI（核心功能完整）/ 平台 AI / BYOK 自带模型；BYOK Key Fernet 加密落库、脱敏回显、可删除、备用提供商 failover；AI 研究辅助（问题改写/反证提示三视角/事实观点拆分），输出一律标注"模型辅助，不是事实来源"。
- **量化能力**：回测/选股/预警在腾讯实时数据层就绪后诚实重启用；回测引擎 v2（佣金/印花税/滑点/最低佣金、盘中止损止盈、夏普/盈亏因子/平均持仓）；海龟突破与动量策略；技术指标（ZIG/PEAK 等，强制未来函数警示，禁作实时信号）；ETF 价格口径跟踪误差代理。
- **本地知识库**：txt/md/docx/pdf 索引（docx=zip+xml 纯标准库，PDF 用可选依赖 pypdf 本地解析），关键词+二元组检索、命中片段高亮，另有人工策展的内置操作指引层与用户文档区分来源；文件不进 git、不外发。
- **移动端导航与布局**：底部高频入口+“更多”完整导航，44/48px 触控目标、ESC/遮罩关闭、安全区与 reduced-motion 适配；宽表自动卡片化。
- **性能首屏分级**：今日页快层（指数+周期+矛盾）先行渲染，宏观/情报慢层骨架填充，次要卡片空闲加载；ECharts 本地化并移至 body 末尾。
- **首屏加载优化**：图表库与 SPA 脚本使用 `defer` 并保持依赖顺序，避免大型脚本阻塞 HTML 解析。
- **仪表盘请求调度**：首屏快层请求在 ticker 与仪表盘之间共享，宏观/情报慢层延后至首屏空闲后加载；自动刷新仍按 30 秒获取新数据。
- **价格预警**：登录后在线每 60 秒轮询一次触发状态，页面内提示与可选浏览器通知去重；明确标注为非推送式监控，退出登录时停止轮询。
- **研究工作台**：以任务入口统一进入标的研究、主题研究、反证自查、复盘、知识检索和 AI 研究辅助；观察台与异动雷达入口联动；不改变无 AI 模式和可信数据边界。

历史版本工程决策记录见 [docs/ROX3-merge-analysis.md](docs/ROX3-merge-analysis.md)。
...
ROX 将新闻专题主线、热度排序、关键词关注、突发事件置顶、全球变量聚合和多来源信息组织为可核验的研究线索。

系统不输出次日涨跌预测、未经验证的情绪置信度或交易营销式结论，而是将情报转化为：
...
- 多周期 MACD 状态卡：日/周/月/季四周期状态与最近交叉并排展示（年线样本不足时如实标注不可用）
- 基于有效输入的确定性多维分析
...
- 桌面、平板和手机布局；移动端为**左右布局**：左侧抽屉导航 + 右侧内容
- “战略文房”暖墨与朱砂视觉系统
- 抽屉导航：44/48px 触控目标、遮罩/ESC 关闭、路由跳转自动收起、安全区与 reduced-motion 适配；桌面端保持常驻侧栏
- 宽表格（持仓/自选）在 ≤768px 自动转为带字段标签的卡片
- 加载、空数据和接口失败状态
- 浏览器缩放与键盘可访问性（focus-visible 焦点环、抽屉关闭时对读屏不可达）

## 移动端现状与适配计划

当前状态（v4.22+）：移动端已切换为左右布局（左侧抽屉导航），触控目标、安全区、可达性按 ui-ux-pro-max 规则清单验收；持仓/自选宽表已卡片化；基金覆盖矩阵等高级页面已回归。剩余优化项见表格。
...
| `SMTP_HOST` | 未设置 | 找回密码/邮箱验证的 SMTP 服务器；未配置时该功能如实提示"未配置" |
| `SMTP_PORT` | `465` | SMTP 端口；465 走 SSL，587 走 STARTTLS |
| `SMTP_USER` | 未设置 | SMTP 登录用户 |
...
| POST | `/api/auth/forgot-password` | 请求密码重置邮件（统一响应，防用户名枚举） |
| POST | `/api/auth/reset-password` | 凭邮件令牌重置密码（令牌单次使用，30 分钟有效） |
| POST | `/api/auth/change-password` | 登录态修改密码 |
| POST | `/api/auth/email/bind` | 绑定/更换邮箱并发送验证邮件 |
| POST | `/api/auth/email/verify` | 凭邮件令牌验证邮箱（24 小时有效） |
| GET | `/api/auth/recovery-status` | 邮件找回能力是否已配置 |
...
| GET | `/api/stock/{code}/macd-matrix` | 多周期 MACD 状态矩阵（历史状态描述，非信号） |
...
费用模型：双边佣金、卖出印花税、不利方向滑点、最低佣金、盘中止损止盈（触发价成交）；
绩效指标：总/超额收益、最大回撤、夏普、盈亏因子、平均盈亏与持仓天数、总费用。
策略库：均线交叉 / RSI 超卖 / 布林突破 / MACD / 海龟突破（Donchian）/ 动量。

| 方法 | 路径 | 用途 |
| --- | --- | --- |
| GET | `/api/backtest/strategies` | 回测策略库 |
| GET | `/api/backtest/stocks` | 可回测股票列表 |
| POST | `/api/backtest/run` | 运行回测（支持费用/止损止盈参数） |

### v4.x 新增 API 摘览

| 方法 | 路径 | 用途 |
| --- | --- | --- |
| POST | `/api/research/{id}/evidence` | 证据抽屉：追加事实/反证/待验证 |
| GET | `/api/research/{id}/detail` | 研究档案（关联决策/胜率/复核提醒） |
| GET | `/api/research/templates[/{id}]` | 研究卡模板（产业链三问/反模式自查） |
| GET | `/api/data/sources` | 数据源健康（真实请求埋点） |
| GET | `/api/intelligence/feed` | 按研究卡/自选关联度排序的情报流 |
| GET | `/api/intelligence/concentration` | 成交额集中度温度计（自建历史） |
| GET | `/api/stock/{code}/pivots` | ZIG 峰谷转折点（含未来函数警示） |
| GET/POST | `/api/knowledge/{status,search,rebuild}` | 本地知识库索引与检索（含内置操作指引与来源区分） |
| GET | `/api/knowledge/playbooks` | 内置操作指引清单（人工策展的方法论蒸馏） |
| POST | `/api/ai/research-assist` | AI 问题改写/反证提示/事实观点拆分 |
| GET/PUT/DELETE | `/api/settings/`（+`/ai-key`） | AI 三层模式与 BYOK 加密 Key 管理 |
| GET | `/api/dashboard/overview/slow` | 首屏分级：宏观/情报慢层 |
...
现有 240 个后端测试覆盖（含找回密码全链路、防枚举一致性、令牌生命周期、AI 三层模式、数据契约、知识库、集中度计算），并由 Playwright E2E 与 Lighthouse 移动审计补充前端质量检查：
...
发布前建议至少执行以上四类检查，并验证 `/health`、`/ready`、仪表盘、个股页和手机底部导航。推送到 `main` 分支后，GitHub Actions 会自动运行 pytest、flake8 和前端 JS 语法检查，Render 随之自动部署（`/health` 可核对线上版本号）。
...
当前 API 已具备多用户 JWT 鉴权、账号级数据隔离与接口限流（全局 200 次/分钟、登录 5 次/分钟）。账号找回链路：绑定并验证邮箱后可自助重置密码（令牌哈希落库、单次使用、30 分钟有效、统一响应防用户名枚举），密码变更/重置后旧 JWT 一律失效。RBAC 仍在路线图中，正式收费服务前需补齐并完成合规审核。
...
| 用户体系 | 已支持注册、登录（JWT + PBKDF2 哈希）、找回密码、邮箱绑定验证与账号级数据隔离；RBAC 待实现 |
...
| 数据备份 | 提供手动完整导出与 GitHub Actions 定时 pg_dump 工作流（需配置 secret）；未做恢复演练 |
| 移动端 | 核心路径已完成 UI 2.0 适配；375/390/414px 已做浏览器验收 |
| 测试 | 287 个后端测试、15 个 Playwright 移动端 E2E，并接入 Lighthouse 移动审计 |
| 基金/ETF | 已支持 ETF 透视、价格风险指标、证据覆盖矩阵与关联决策；净值/IOPV/持仓/跟踪误差待可靠数据源 |
| 研究卡 | 已支持生命周期、假设状态、下次复核日期、决策关联和跨页面证据抽屉；模板含产业链三问、反模式自查、三流纪律、多周期趋势自查、最坏情况先行 |
| 知识库 | txt/md/docx/pdf 本地索引与检索（PDF 依赖 pypdf 本地解析）；关键词匹配而非语义检索（诚实标注）；内置 10 条人工策展操作指引 |
| 主题 | 支持 light/dark 切换并持久化，图表配色随主题更新 |
| 商业合规 | 用户协议 / 隐私政策 / 风险揭示 v1.0 已上线（免费公测模板，正式收费前需专业法律审阅）；支付与会员未接入 |
...
- ✅ 邮箱验证和密码重置（v4.44.0；绑定并验证邮箱后自助找回）
