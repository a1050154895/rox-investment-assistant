...
## 当前版本状态（v4.45.0 · 2026-09-06）

> 253 项后端测试、15 项移动端 E2E 全绿，Render 线上持续部署。以下为 v4.9.0 以来的主要升级：

- **v4.45 认知工具与知识库增强**：个股透视新增「多周期 MACD 状态」卡（日/周/月/季各周期独立描述金叉死叉与最近交叉，季线由月线按日历季度聚合，年线样本不足如实标注不可用，只描述历史不构成信号）；研究卡新增「多周期趋势状态自查」「最坏情况先行」两个模板；知识库支持 PDF 索引（pypdf 本地解析）、检索结果带关键词高亮，并内置 8 条人工策展的「操作指引」（与用户文档明确区分来源，可在工作台一键存为速记）。方法论蒸馏与思想来源登记见 docs/methodology.md 与 docs/strategy_origins.md。
...
| 测试 | 253 个后端测试、15 个 Playwright 移动端 E2E，并接入 Lighthouse 移动审计 |
...
| 知识库 | txt/md/docx/pdf 本地索引与检索（PDF 依赖 pypdf 本地解析）；关键词匹配而非语义检索（诚实标注）；内置 8 条人工策展操作指引 |
