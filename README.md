...
## 当前版本状态（v4.44.0 · 2026-09-05）

> 240 项后端测试、15 项移动端 E2E 全绿，Render 线上持续部署。以下为 v4.9.0 以来的主要升级：
...
- **本地知识库**：txt/md/docx 纯标准库索引（docx=zip+xml），关键词+二元组检索，文件不进 git、不外发。
...
| GET/POST | `/api/knowledge/{status,search,rebuild}` | 本地知识库索引与检索 |
...
| 测试 | 240 个后端测试、15 个 Playwright 移动端 E2E，并接入 Lighthouse 移动审计 |
| 基金/ETF | 已支持 ETF 透视、价格风险指标、证据覆盖矩阵与关联决策；净值/IOPV/持仓/跟踪误差待可靠数据源 |
| 研究卡 | 已支持生命周期、假设状态、下次复核日期、决策关联和跨页面证据抽屉 |
